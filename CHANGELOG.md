...
- CI (`.github/workflows/ci.yml`): `build-and-test` now builds/tests with `xcodebuild` against an iOS Simulator destination instead of bare `swift build`/`swift test` (which targets macOS). The package is iOS-only in intent — `SwiftCoreWebDashboard` imports `UIKit` — so a macOS-targeted build always failed with `no such module 'UIKit'`, independent of any source fix.
- CI: switched `runs-on` to the `xcode-27` runner image (Xcode 27.0 build 27A266a) so the job's toolchain matches what's verified locally via `scripts/ci-local.sh`, instead of chasing simulator/runtime mismatches on `macos-15`.
- `LifecycleSupport.swift`: `requestSingleAppMode()` is now `async` and uses `withCheckedContinuation` instead of a `DispatchSemaphore` + captured `var` (Swift 6 strict concurrency rejects mutating a captured var from the completion closure, which runs on a different isolation domain).
